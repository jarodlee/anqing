...
<center>
<br><br><br><br>
<font color="<% =strForumFontColor %>" face="<% =strDefaultFontFace %>" size="<% =strDefaultFontSize %>"><B>只有登陆了论坛才能阅读悄悄话信息</b></font>
</center>
<% else
...
function members2() { var MainWindow = window.open ("members2.asp", "","toolbar=no,location=no,menubar=no,scrollbars=yes,width=250,height=500,resizeable=yes,status=no");
}
function ignorelist() { var MainWindow = window.open ("pm_ignore.asp", "","toolbar=no,location=no,menubar=no,scrollbars=yes,width=250,height=500,resizeable=yes,status=no");
}
// done hiding -->
</script>
<center>
<table border="0" width="100%" align="center">
  <tr>
    ...
    <img src="<%=strImageURL %>icon_folder_open.gif" height=15 width=15 border="0">&nbsp;<a href="default.asp">返回论坛首页</a>&nbsp;<img src="<%=strImageURL %>icon_folder_open.gif" height=15 width=15 border="0">&nbsp;<a href="pm_view.asp">悄悄话讯息</a>&nbsp;<img src="<%=strImageURL %>icon_folder_open_topic.gif" height=15 width=15 border="0">&nbsp;阅读悄悄话信息<br>
    </font></td>
  </tr>
</table>
...
<tr>
<td colspan=2 align="right"><A HREF="pm_view.asp"><img src="<%=strImageURL %>pmreceive.jpg" BORDER=0 ALT="检查是否有新悄悄话"></A><A HREF="privatesend.asp?method=Reply&REPLY_ID=<% =rs("MEMBER_ID")%>&id=<% =Request.QueryString("id")%>&R_SUBJECT=<% =ChkString(rs("M_SUBJECT"),"urlpath")%>"><img src="<%=strImageURL %>pmreply.jpg"  BORDER=0 ALT="回复悄悄话"></A><a href="privatesend.asp?method=Forward&REPLY_ID=<% =rs("MEMBER_ID")%>&id=<% =Request.QueryString("id")%>&R_SUBJECT=<% =ChkString(rs("M_SUBJECT"),"urlpath")%>""><img src="<%=strImageURL %>pmforward.jpg" BORDER=0 ALT="转发此悄悄话"></A><A HREF="privatedelete.asp?method=DeleteMessage&id=<%=Request.QueryString("id")%>"><img src="<%=strImageURL %>pmdelete.jpg" BORDER=0 ALT="删除此悄悄话"></A><A HREF="javascript:ignorelist()"><img src="<%=strImageURL %>pmignore.jpg"  BORDER=0 ALT="把 <% =rs("M_NAME") %> 加入黑名单"></A></td>
</tr></table>
...
<img src="<%=strImageURL %>icon_posticon.gif" border="0" hspace="3"><font color="<% =strForumFontColor %>" face="<% =strDefaultFontFace %>" size="<% =strFooterFontSize %>"> <% =ChkDate(rsMessage("M_SENT")) %>&nbsp;&nbsp;<% =ChkTime(rsMessage("M_SENT")) %></font>
	...
	&nbsp;<a href="privatesend.asp?method=ReplyQuote&REPLY_ID=<% =rs("MEMBER_ID")%>&id=<% =Request.QueryString("id")%>&R_SUBJECT=<% =ChkString(rs("M_SUBJECT"),"urlpath")%>"><img src="<%=strImageURL %>1-quote.gif" alt="回复并引用原文" border="0" align="absmiddle" hspace="6"></a>
...
<%			if strICQ = "1" then %>
<%			  if rs("M_ICQ") <> " " then %>
        &nbsp;<a href="JavaScript:openWindowICQ('pop_messengers.asp?mode=ICQ&ICQ=<% =rs("M_ICQ") %>&M_NAME=<% =ChkString(rs("M_NAME"),"urlpath") %>')"><img src="http://online.mirabilis.com/scripts/online.dll?icq=<% =ChkString(rs("M_ICQ"), "urlpath") %>&img=5" height=15 width=15 alt="发送ICQ讯息给 <% = ChkString(rs("M_NAME"),"display")  %>" border="0" align="absmiddle" hspace="6"></a>
<%			  end if %>
<%			end if %>
<%			if strYAHOO = "1" then %>
<%			  if rs("M_YAHOO") <> " " then %>
