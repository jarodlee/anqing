...
child=rs(4)+1
i=0
set rs=conn.execute("select Boardid from board where boardid="&newboard&" and ParentStr like '%"&ParentStr&"%'")
if not (rs.eof and rs.bof) then
	response.write "不能将一个版面合并到其下属论坛中"
	exit sub
end if
'得到当前版面下属论坛ID
set rs=conn.execute("select Boardid from board where ParentStr like '%"&ParentStr&"%'")
if not (rs.eof and rs.bof) then
do while not rs.eof
