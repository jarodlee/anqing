...
if membername<>request("username") then 
	...
	if Cint(forum_setting(48))=1 then
...
dim caneditpost
caneditpost=false
sql="select b.username,b.dateandtime,u.usergroupID from "&TotalUseTable&" b,[user] u where b.postuserid=u.userid and b.AnnounceID="&AnnounceID
...
if rs.eof and rs.bof then
	...
	if Clng(forum_setting(50))>0 then
		if Datediff("s",rs("dateandtime"),Now())>Clng(forum_setting(50))*60 then
			Body=addbody+chr(13)+chr(10)+char_changed+chr(13)
		else
			Body=addbody
		end if
	else
...
if not (rs.eof and rs.bof) then
	if not isnull(rs(0)) and rs(0)<>"" then
		...
		if ubound(LastBoard)=7 then
			...
			LastPost=LastBoard(0) & "$" & LastBoard(1) & "$" & Now() & "$" & replace(cutStr(Topic,20),"$","") & "$" & LastBoard(4) & "$" & LastBoard(5) & "$" & LastBoard(6) & "$" & boardid
			conn.execute("update board set LastPost='"&reubbcode(replace(LastPost,"'",""))&"' where boardid="&boardid)
			end if
		end if
	end if
end if
...
if not (rs.eof and rs.bof) then
	if not isnull(rs(0)) and rs(0)<>"" then
		...
		if ubound(LastTopic)=7 then
			...
			LastPost=LastTopic(0) & "$" & LastTopic(1) & "$" & Now() & "$" & replace(cutStr(body,20),"$","") & "$" & LastTopic(4) & "$" & LastTopic(5) & "$" & LastTopic(6) & "$" & boardid
			conn.execute("update topic set LastPost='"&reubbcode(replace(LastPost,"'",""))&"' where topicid="&rootid)
			end if
		end if
	end if
end if
