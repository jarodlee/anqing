...
%>
<script>
	parent.followTd<%=rootid%>.innerHTML='<TABLE border=0 cellPadding=0 cellSpacing=0 width="100%" align=center><TBODY><%do while not rs.eof%><TR><TD class=tablebody1 width="100%" height=25><%for i=2 to rs("layer")%><%=outtext%><%next%><img src="<%=Forum_info(7)&Forum_boardpic(16)%>"><a href="dispbbs.asp?boardID=<%=boardID%>&ID=<%=rs("RootID")%>&replyID=<%=rs("announceID")%><%
