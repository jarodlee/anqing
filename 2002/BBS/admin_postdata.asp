...
<%
	...
	dim admin_flag
	admin_flag="34,35"
	if not master or instr(session("flag"),"34")=0 or instr(session("flag"),"35")=0 then
...
sub del()
	...
	conn.execute("delete from BestTopic where RootID in (select TopicID from topic where PostTable='"&request("tablename")&"')")
	conn.execute("delete from Topic where PostTable='"&request("tablename")&"'")
	response.write "删除成功！"
end sub
